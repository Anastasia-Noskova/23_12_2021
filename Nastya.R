...
#данные ближайших метеостаций к парку ЦПКиО г.Курган
Kurgan = data.frame(id="KGN",
                    latitude = c(55.430755),
                    longitude= c(65.326146))

station_list = meteo_nearby_stations(lat_lon_df = Kurgan, 
...
station_list = station_list %>% filter(name %in% c("KURGAN")) #названия станций


#Чтобы получить все данные с метеостации, нужно 
one_station = meteo_tidy_ghcnd("RSM00028661") 
...
Kurgan_cum = all_data %>% mutate(month = month(date)) %>% 
  filter(month > 4 & month < 10) %>% 
  group_by(year) %>% 
  mutate(prcp_cum = cumsum(prcp))

Kurgan_cum %>% summarise(prcp_avg = max(prcp_cum), n = n())
...
park_sf <- read_sf(str_interp("${getwd()}/Kurgan.geojson"))
...
dates = MODISTools::mt_dates(product = "MOD16A2", lat = 55.430755, lon = 65.326146)
...
Kurgan_ET =  MODISTools::mt_subset(product = "MOD16A2",
                                      lat = 55.430755,
                                      lon = 65.326146,
                                      band = "ET_500m",
                                      start = "2018-05-01", 
...
# 
Kurgan_ET = Kurgan_ET %>% filter(value < 32700) %>% dplyr::select(units,calendar_date,value) %>%
...
ggplot(Kurgan_ET, aes(x=doy,y=ET))+
  geom_point()+
  geom_smooth()+
...
ggplot(Prcp_cum, aes(x = doy,y = ETcum))+
  ...
  ylab("ET vs Precipitation,m3 for Kurgan park, 2018")+ 
  theme_bw()
